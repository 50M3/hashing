...
int main() {
	...
	try {
		while (true) {
			...
			cout << "5 -> Testrutiene ausfuehren!" << endl;
			...
			case 5:
				cout << "-Testrutieren wird ausgefuehrt!" << endl;
				//int* randomValues;
				...
				ofstream myfile("Graphen.csv", ios::out | ios::app);
				if(myfile.is_open())
				{
				myfile << (double)MAX_HASH / (double)TABLE_SIZE;
				myfile << ",";
				myfile <<anzahl.erfolg;
				myfile << ",";
				myfile << anzahl.erfolglos;
				myfile <<"\n";
				myfile.close();
				}
				//delete_key(4);
